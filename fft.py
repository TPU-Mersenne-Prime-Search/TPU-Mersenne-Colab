Pp# Copyright 2024 The swirl_lm Authors.
#
...
class DistFFT():
  ...
  * Usage example for operating on partitioned input (within partition):
  ...
  def fft_1d(self,
    ...
    where `partition_n` is the number of partitions along the axis `n`
    and `local_n` is the size (including of the margin on both sides) of the
    partitioned output in the axis `n`.


    Args:
      global_input: A 3D array representing the unpartitioned input for the fft.
    ...
    split_output = do_transform(global_input_reshaped, axis, inverse, margin_width)
...
#    print(gathered_input.shape)
    ...
    local_fft = (
      jnp.fft.ifft(gathered_input) if inverse else jnp.fft.fft(
        gathered_input))
  ...
  def fft(self, xs, inverse=False):
    return self.fft_1d(jnp.expand_dims(jnp.expand_dims(xs, axis=0), axis=0), axis=2, inverse=inverse)[0][0]
